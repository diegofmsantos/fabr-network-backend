// scripts/gerar-resultados-por-fim-de-semana.ts
// Script para gerar planilhas de RESULTADOS separadas por fim de semana
...
interface JogoResultado {
  id_jogo: number  // ✅ CORRIGIDO: era 'id', agora 'id_jogo'
...
// ✅ GERAÇÃO DE PLACARES REALISTAS
function gerarPlacarRealista(): { mandante: number; visitante: number } {
  // Pontuações comuns no futebol americano (múltiplos de 3 e 7 principalmente)
  ...
  // Evitar empates em 90% dos casos (futebol americano raramente empata)
  if (mandante === visitante && Math.random() < 0.9) {
    const incrementos = [3, 7, 6] // Preferir incrementos realistas
...
// ✅ BUSCAR JOGOS DA AGENDA AGRUPADOS POR FIM DE SEMANA
async function buscarJogosAgrupados(): Promise<Map<number, any[]>> {
  console.log('🔍 Buscando agenda da Superliga 2025...')
  ...
  // Buscar TODOS os jogos da agenda
  ...
  // ✅ AGRUPAR JOGOS POR FIM DE SEMANA (DATAS PRÓXIMAS JUNTAS)
  ...
  jogos.forEach((jogo) => {
    ...
    if (dataAnterior) {
      ...
      // Isso agrupa jogos de sábado/domingo/segunda no mesmo fim de semana
      if (diferencaDias > 3) {
        fimDeSemanaAtual++
      }
    }
  ...
  // Log da distribuição
  console.log('\n📊 DISTRIBUIÇÃO DOS JOGOS POR FIM DE SEMANA:')
  ...
  console.log(`\n🎯 Total de fins de semana: ${jogosPorFimDeSemana.size}`)

  return jogosPorFimDeSemana
}

// ✅ GERAR RESULTADOS FAKE PARA UM FIM DE SEMANA
async function gerarResultadosFimDeSemana(fimDeSemana: number, jogos: any[]): Promise<JogoResultado[]> {
  ...
  for (const jogo of jogos) {
    ...
    const resultado: JogoResultado = {
      id_jogo: jogo.id,  // ✅ CORRIGIDO: era 'id', agora 'id_jogo'
      ...
      data_jogo: new Date(jogo.dataJogo).toISOString().split('T')[0], // YYYY-MM-DD
...
// ✅ CRIAR PLANILHA DE RESULTADOS PARA UM FIM DE SEMANA
async function criarPlanilhaResultados(fimDeSemana: number, resultados: JogoResultado[]): Promise<string> {
  ...
  const info = [
    ['📋 SUPERLIGA 2025 - RESULTADOS DOS JOGOS'],
    [''],
    ...
    [`estatisticas_fim_de_semana_${String(fimDeSemana).padStart(2, '0')}_${dataJogo.replace(/-/g, '')}.xlsx`]
  ]
  ...
  const nomeArquivo = `resultados_fim_de_semana_${String(fimDeSemana).padStart(2, '0')}_${dataFormatada}.xlsx`
  
  // ✅ GARANTIR QUE A PASTA EXISTE
  const pastaDestino = 'planilhas-resultados-fins-de-semana'
...
// ✅ GERAR TODAS AS PLANILHAS DE RESULTADOS
async function gerarTodasAsPlanilhasResultados(): Promise<void> {
  console.log('🚀 INICIANDO GERAÇÃO DE PLANILHAS DE RESULTADOS POR FIM DE SEMANA\n')
  
  try {
    // Buscar jogos agrupados por fim de semana
    ...
    // ✅ RELATÓRIO FINAL
    console.log('\n🎉 GERAÇÃO DE RESULTADOS COMPLETA!')
    ...
    console.log('4. O sistema deve gerar playoffs automaticamente')
    console.log('')
    console.log('🔜 PRÓXIMO PASSO: Execute o script de gerar estatísticas')
    console.log('npm run generate:estatisticas-fins-de-semana')
...
async function gerarFimDeSemanaEspecifico(numero: number): Promise<void> {
  console.log(`🎯 Gerando RESULTADOS apenas do Fim de Semana ${numero}...`)
...
async function main() {
  try {
    ...
    if (args.includes('--help') || args.includes('-h')) {
      console.log('📖 USO DO SCRIPT DE RESULTADOS:')
      console.log('')
      console.log('npm run generate:resultados-fins-de-semana              # Gerar todos os fins de semana')
      console.log('npm run generate:resultados-fins-de-semana --fs=N       # Gerar apenas fim de semana N')
      console.log('npm run generate:resultados-fins-de-semana --help       # Mostrar ajuda')
      console.log('')
      console.log('EXEMPLOS:')
      console.log('npm run generate:resultados-fins-de-semana --fs=1       # Apenas Fim de Semana 1')
      console.log('npm run generate:resultados-fins-de-semana --fs=17      # Apenas Fim de Semana 17')
      console.log('')
      console.log('⚠️ IMPORTANTE: Execute a agenda primeiro!')
      console.log('⚠️ Para estatísticas, use: npm run generate:estatisticas-fins-de-semana')
      return
    }
...
export { gerarTodasAsPlanilhasResultados, gerarFimDeSemanaEspecifico }